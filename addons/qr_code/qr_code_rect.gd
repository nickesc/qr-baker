...
func _update_qr() -> void:
    self.texture = ImageTexture.create_from_image(generate_qr_image())

func generate_qr_image():
    if self.auto_module_px_size:
        self.module_px_size = mini(self.size.x, self.size.y) / (self._qr.get_module_count() + 2 * self.quiet_zone_size)
    return self._qr.generate_image(self.module_px_size, self.light_module_color, self.dark_module_color, self.quiet_zone_size)
